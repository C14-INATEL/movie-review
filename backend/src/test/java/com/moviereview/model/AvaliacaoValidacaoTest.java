...
class AvaliacaoValidacaoTest {
    ...
    @Test
    @DisplayName("T02 – setNota deve lançar exceção para nota menor que 1 (incluindo 0)")
    void deveLancarExcecaoParaNotaMenorQueUm() {
        Avaliacao avaliacao = new Avaliacao(1L,
                new Usuario("Test", "test@email.com", "123"),
                new Filme("Film", "Dir", 2000),
                3);

        assertThrows(IllegalArgumentException.class, () -> avaliacao.setNota(0));
        assertThrows(IllegalArgumentException.class, () -> avaliacao.setNota(-1));
    }

    @Test
    @DisplayName("T03 – setNota deve aceitar nota válida entre 1 e 5")
